...
int sys_fork() {
	...
	PCB *pcb = &(PCBs[pcbs_avl]);
	pcb->pid = pcbs_avl;
	pcb->lock_depth = current->lock_depth;
	...
	pcb->sf->p_trap.ebp = current->sf->p_trap.ebp;
...
void sys_sleep(uint32_t time) {
	...
	if (current->state == READY) {
		...
		list_add_tail(&(current->state_list),&blockq_h);
	}
	schedule();
	unlock();
}
...
void wake_up(PCB *block) {
	lock();
	if(block->state == SLEEP) {
		block->state = READY;
		//wake up block pcb
		list_del(&(block->state_list));
		list_add_tail(&(block->state_list),&readyq_h);
	}
	schedule();
	unlock();
}
...
void sys_exit(int status) {
	...
	if(current->state == READY) {
		...
		//exit current pcb
		list_del(&(current->state_list));
		list_add_tail(&(current->state_list),&freeq_h);
	}
	schedule();
	unlock();
}
